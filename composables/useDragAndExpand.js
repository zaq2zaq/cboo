...
export function useDragAndExpand(searchActive) {
    ...
    const updateSearchPanelHeight = () => {
        nextTick(() => {
            ...
            if (panel) searchPanelHeight.value = panel.offsetHeight;
        });
    };
    const dynamicContainer2Top = computed(() => {
        if (searchActive.value) {
            const panel = document.querySelector('.search-panel');
            if (panel) return panel.getBoundingClientRect().bottom;
            const kpi = document.querySelector('.kpi-flat-row');
            if (kpi) return kpi.getBoundingClientRect().bottom + 200;
            return 200;
        }
        return container2Top.value;
    });
    watch(searchActive, (val) => {
        if (val) updateSearchPanelHeight();
        else {
            // 关闭搜索时重新计算位置
            computeAndSetPosition();
        }
    ...
    return {
        isExpanded, isDragging, blueCardMarginTop, container2Top, container2Height, scrollableRef,
        onTouchStartDrag, onTouchMoveDrag, onTouchEndDrag,
        onScrollableTouchStart, onScrollableTouchMove, onScrollableTouchEnd,
        toggleExpand, forceCollapse,
        computeAndSetPosition,
        searchPanelHeight, updateSearchPanelHeight, dynamicContainer2Top
    };
}
