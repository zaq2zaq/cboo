    ...
    setup() {
        ...
        async function toggleSearch() {
            ...
            if (!modalState.searchActive.value) {
                ...
                // 位置更新由 transition 的 after-leave 钩子负责
            }
        }
        ...
        async function saveProduct() {
            ...
            const colors = modalState.newProduct.value.colors.map(c => { 
                const stockMap = {}; 
                const sizes = utils.parseSizeRange(modalState.newProduct.value.sizeRange);
                for (let size of sizes) stockMap[size] = 0;
                return { ...c, sizeRange: modalState.newProduct.value.sizeRange, stockMap }; 
            });
        ...
        provide('getProductTotalStock', utils.getProductTotalStock.bind(utils)); 
        provide('parseSizeRange', utils.parseSizeRange.bind(utils)); 
        provide('displaySize', utils.displaySize.bind(utils));
        provide('getColorTotal', utils.getColorTotal.bind(utils)); 
        provide('getStockForColor', utils.getStockForColor.bind(utils)); 
        provide('filteredCount', productData.filteredCount);
        
        return {
            searchActive: modalState.searchActive, sidebarOpen: modalState.sidebarOpen, isExpanded: dragExpand.isExpanded, isDragging: dragExpand.isDragging, blueCardMarginTop: dragExpand.blueCardMarginTop, container2Top: dragExpand.container2Top, container2Height: dragExpand.container2Height, dynamicTop: dragExpand.dynamicTop, scrollableRef: dragExpand.scrollableRef,
            ...
            onSearchPanelAfterEnter: dragExpand.onSearchPanelAfterEnter,
            onSearchPanelAfterLeave: dragExpand.onSearchPanelAfterLeave,
        };
    },
    template: `
        <div class="app-wrapper">
            <div class="container-2" :class="{ 'expanded-down': isExpanded, dragging: isDragging }" :style="{ top: dynamicTop + 'px', height: container2Height + 'px' }">
                <div class="drag-handle" :class="{ 'search-mode': searchActive }">
            ...
            <transition name="search-panel" @after-enter="onSearchPanelAfterEnter" @after-leave="onSearchPanelAfterLeave">
