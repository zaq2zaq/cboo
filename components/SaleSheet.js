export const SaleSheet = {
    ...
    setup() {
        // 注入 displaySize 函数，用于将真实尺码（34）显示为个位数（4）
            ...
            <div class="bottom-sheet" @click.stop>
                ...
                <div class="sheet-title">
                    <div class="title-left">
                        <i class="fas fa-arrow-up title-icon"></i>
                        <span class="title-prefix">出库·</span>
                        <span class="title-product">{{ currentProduct?.sku }} {{ currentProduct?.name }}</span>
                    </div>
                    <button class="select-customer-btn" @click.stop="$emit('openCustomerSelector')">
                        <i class="fas fa-user-friends"></i>
                        <span class="selected-customer-name">{{ selectedCustomer ? (selectedCustomer.name.length > 4 ? selectedCustomer.name.slice(0,4)+'…' : selectedCustomer.name) : '选择客户' }}</span>
                    </button>
                </div>
                <div class="sheet-scrollable">
                    <div v-for="(color, idx) in currentProduct?.colors" :key="color.name" class="color-block">
                        ...
                        <div class="size-grid">
                            <!-- 关键修复：v-for 遍历 size，v-model 绑定到 tempSaleData[color.name][size] -->
                            <div v-for="size in $emit('parseSizeRange', color.sizeRange)" :key="size" class="size-input-item">
                                <div class="size-left" @click="$emit('adjustQuantity', color.name, size, -1)">{{ displaySize(size) }}码</div>
                                <input type="number" v-model.number="tempSaleData[color.name][size]" class="size-input" min="0" :max="$emit('getStockForColor', color, size)" step="1" @input="$emit('validateQuantity', color.name, size)">
                                <div class="size-right sale" @click="$emit('adjustQuantity', color.name, size, 1)">余 {{ $emit('getStockForColor', color, size) - (tempSaleData[color.name][size] || 0) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="sheet-footer">
                    <details class="action-panel">
                        <summary>
                            <div class="summary-left">快捷操作</div>
                            <div class="summary-right" v-if="saleTotalQuantity > 0">共 {{ saleTotalQuantity }} 双 | 总价 ¥{{ saleTotalAmount }}</div>
                        </summary>
