            ...
            <div class="bottom-sheet" @click.stop>
                ...
                <div class="sheet-title">
                    <div class="title-left">
                        ...
                        <span class="title-prefix">入库·</span>
                ...
                <div class="sheet-scrollable">
                    <div v-for="(color, idx) in currentProduct?.colors" :key="color.name" class="color-block">
                        ...
                        <div class="size-grid">
                            <!-- 关键修复：v-for 遍历 size，v-model 绑定到 tempStockInData[color.name][size] -->
                            <div v-for="size in $emit('parseSizeRange', color.sizeRange)" :key="size" class="size-input-item">
                                <div class="size-left" @click="$emit('adjustStockInQuantity', color.name, size, -1)">{{ displaySize(size) }}码</div>
                                <input type="number" v-model.number="tempStockInData[color.name][size]" class="size-input" min="0" step="1" @input="$emit('validateStockInQuantity', color.name, size)">
                                <div class="size-right in" @click="$emit('adjustStockInQuantity', color.name, size, 1)">现 {{ $emit('getStockForColor', color, size) + (tempStockInData[color.name][size] || 0) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="sheet-footer">
                    <details class="action-panel">
                        <summary>
                            ...
                            <div class="summary-right" v-if="stockInTotalQuantity > 0">共 {{ stockInTotalQuantity }} 双</div>
                        </summary>
