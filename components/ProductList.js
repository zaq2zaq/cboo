export const ProductList = {
    ...
    setup() {
        ...
        return { getProductTotalStock, parseSizeRange, displaySize, getColorTotal, getStockForColor };
    },
    template: `
                    ...
                    <div class="size-detail" :class="{ show: showDetail[product.id] }" @click.stop>
                        <div class="size-table">
                            ...
                            <div v-for="color in product.colors" class="color-row"><div class="color-name"><span>{{ color.name }}</span><span class="color-total">{{ getColorTotal(color) }}双</span></div>
                                <div class="size-badge-group"><div v-for="size in parseSizeRange(color.sizeRange)" class="size-badge"><span class="digit">{{ displaySize(size) }}</span><span class="slash">/</span><span class="stock">{{ getStockForColor(color, size) }}</span></div></div>
                            </div>
                        </div>
                    </div>
                </div>
